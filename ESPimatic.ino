...
long vallox_checkInterval = 5000; // in millis
long vallox_lastInterval = 0;
int vallox_position = 1;
...
void setup()
{
  ...
  ValloxEnabled = HandleEeprom(enablevallox_Address, "read");
  if (ValloxEnabled == "1")
  {
    String vallox_pin = HandleEeprom(valloxpin_Address, "read");
    pinMode(vallox_pin.toInt(), OUTPUT);
  } 
  ...
  server.on("/vallox_ajax", handle_vallox_ajax);
...
void handle_api()
{
  ...
  {
    Serial.println("/api?action=" + action + "&value=" + value);
    ...
    {
      int val = value.toInt();
      set_vallox_switch_position(val);
      server.send(200, "text/html", "OK");
    }
  }
}
...
void handle_vallox_ajax()
{
  ...
  else
  {
    ...
    if (form != "vallox")
    {
      String vallox_enable = HandleEeprom(enablevallox_Address, "read");
      String vallox_pin = HandleEeprom(valloxpin_Address, "read");
      String vallox_value = HandleEeprom(valloxvalue_Address, "read");

      String vallox_listbox = "";
      if (vallox_pin != "")
      {
        vallox_listbox = HWListBox(0, 16, vallox_pin.toInt(), "vallox_pin", "vallox");
      }
      else
      {
        vallox_listbox = HWListBox(0, 16, -1, "vallox_pin", "vallox");
      }

      // Glue everything together and send to client
      server.send(200, "text/html", vallox_enable + sep + vallox_listbox + sep + vallox_value + sep + ErrorList[ErrorWifi] + sep + ErrorList[ErrorEeprom] + sep + ErrorList[ErrorDs18b20] + sep + ErrorList[ErrorUpgrade]);
    }
    if (form == "vallox")
    {
      String vallox_boolArg = server.arg("vallox_bool");
      String vallox_pinArg = server.arg("vallox_pin");
      String vallox_valueArg = server.arg("vallox_value");

      if (vallox_boolArg == "on")
      {
        vallox_boolArg = "1";
      }
      else
      ...
      HandleEeprom(enablevallox_Address, "write", vallox_boolArg);
      HandleEeprom(valloxpin_Address, "write", vallox_pinArg);
      HandleEeprom(valloxvalue_Address, "write", vallox_valueArg);
...
int get_vallox_switch_position()
{
  String value = HandleEeprom(valloxvalue_Address, "read");
  if (value == "")
  {
    return -1;
  }

  int rawVal = value.toInt();
...
void set_vallox_switch_position(int position)
{
  if (position > 0 && position < 5)
  {
    // default is lowest value
    int val = 230;
    if (position == 2)
      val = 560;
    else if (position == 3)
...
void set_vallox(int value)
{
  Serial.println("set_vallox " + value);
  if (value >= 230 && value <= 1023)
  {
    String vallox_pin = HandleEeprom(valloxpin_Address, "read");
    HandleEeprom(valloxvalue_Address, "write", String(value));
    analogWrite(vallox_pin.toInt(), value);
  }
}
...
void loop(void)
{
  ...
  if (millis() - vallox_lastInterval > vallox_checkInterval && ValloxEnabled == "1")
  {
    int position = get_vallox_switch_position();
    if (position > 0 && position != vallox_position)
    {
      vallox_position = position;
      set_vallox_switch_position(position);
    }
    vallox_lastInterval = millis();
  }
...
void CharOnLED(int ch, int led)
{
  ...
  if (ch == '.')
    int FromAlph = 13;
  if ((ch == '('))
    int FromAlph = 14; //replace by 'ñ'
  if ((ch >= '0') && (ch <= '9'))
    int FromAlph = (ch - '0');
  if ((ch >= 'A') && (ch <= 'Z'))
    int FromAlph = (ch - 'A' + 15);
  if ((ch >= 'a') && (ch <= 'z'))
    int FromAlph = (ch - 'a' + 15);
...
{
  String HTML = "";
  int UsedPins[] = {99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99};
  int ds18b20_pos = 0;
  int matrix1_pos = 1;
  int matrix2_pos = 2;
  int matrix3_pos = 3;
  ...
  if (vallox_enable == "1" && Hardware != "vallox")
  {
    UsedPins[vallox_pos] = vallox_pin.toInt();
  }
